...
int8_t LLCP::waitForConnection(uint16_t timeout)
{
    ...
    // Get CONNECT PDU
    Serial.println(F("wait for a CONNECT PDU"));
    do {
    ...
    // Put CC PDU
    Serial.println(F("put a CC(Connection Complete) PDU to response the CONNECT PDU"));
...
int8_t LLCP::connect(uint16_t timeout) {
    ...
    // wait for a CC PDU
    Serial.println(F("wait for a CC PDU"));
    do {
        if (2 > link.read(headerBuf, headerBufLen)) {
            return -1;
        }
